...
\begin{abstract}
本文面向具有范畴论或 topos 背景、但不熟悉现代深度学习算法细节的读者。目标不是介绍工程调参，而是建立一套统一符号，说明监督学习、神经网络、反向传播、卷积网络、图神经网络、Transformer 和大语言模型的基本数学对象。后续若要讨论这些对象能否被组织成范畴、预层、层、stack 或 topos，首先必须知道 AI 侧的对象、态射候选、局部结构和训练目标是什么。
\end{abstract}
...
f:X\to Y.
\]
这个想法有用，但还不够。机器学习中的一个模型通常包含至少四层数据：
\[
\text{数据空间}\quad
\text{参数空间}\quad
\text{损失函数}\quad
...
\]
其中 \(\ThetaSet\) 是参数空间。训练并不是只研究某个固定态射 \(N_\theta\)，而是在 \(\ThetaSet\) 中寻找使损失较小的参数。

\begin{principle}[先分清三种对象]
讨论 AI 与范畴论的关系时，应区分：
\begin{enumerate}
...
\end{enumerate}
很多“神经网络是某种范畴对象”的说法，若不说明指的是这三者中的哪一个，就会含混。
\end{principle}
...
\subsection{反向传播的链式法则}

反向传播不是神秘算法，而是高效组织链式法则。设单个样本的损失为
\[
\Loss=\ell(h_L,y).
\]
定义误差信号
...
GNN 的关键是置换等变性。若重新编号节点，输出表示也应按同样方式重新编号，而不应依赖任意编号。这一点和范畴论中的自然性、等变性很接近，但具体证明依赖聚合函数是否对邻居顺序不敏感。

\section{Transformer 的统一符号}

Transformer 是现代 LLM 的核心。设词表为
\[
\Vocab=\{1,\ldots,V\}.
\]
